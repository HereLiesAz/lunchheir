...
| Feature toggles: consolidated | `HomeScreenPreferences.kt` | one Lunch Heir `SwitchPreference` section with every toggle |
| Feature toggles: contextual | `DockPreferences.kt`, `FolderPreferences.kt`, `GeneralPreferences.kt` | each toggle also under its native category (`LunchHeirFeatureTogglesFor`) |
